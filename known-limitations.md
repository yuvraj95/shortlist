...
A single resume per user. Active seekers applying across multiple role families (e.g., PM and GPM roles simultaneously) cannot maintain separate profiles.
...
A prototype with known limitations that solves a real problem is more valuable than a perfect product that never ships. These limitations are documented, prioritised, and mapped to the roadmap. The core loop — upload, score, filter, triage — works. That is what v0.1 needed to prove.
